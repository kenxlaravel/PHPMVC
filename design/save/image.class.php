...
class Thumb_image {
  ...
  function GenerateThumbFile($from_name, $to_name) {
    ...
    function GenerateMediumThumbFile($from_name, $to_name) {
    ...
    if ($orig_x > $this->max_xm or $orig_y > $this->max_ym) {
 
      // resize
      $per_x = $orig_x / $this->max_xm;
      $per_y = $orig_y / $this->max_ym;
      if ($per_y > $per_x) {
        $this->max_xm = $orig_x / $per_y;
      }
      else {
        $this->max_ym = $orig_y / $per_x;
      }
    ...
    if ($this->image_type == 1) {
      // should use this function for gifs (gifs are palette images)
      $ni = imagecreate($this->max_xm, $this->max_ym);
    }
    else {
      // Create a new true color image
      $ni = ImageCreateTrueColor($this->max_xm,$this->max_ym);
    }
 
    // Fill image with white background (255,255,255)
    $white = imagecolorallocate($ni, 255, 255, 255);
    imagefilledrectangle( $ni, 0, 0, $this->max_xm, $this->max_ym, $white);
    // Create a new image from source file
    $im = $this->ImageCreateFromType($orig_img_type,$from_name);
    // Copy the palette from one image to another
    imagepalettecopy($ni,$im);
    ...
    imagecopyresampled(
      $ni, $im,             // destination, source
      0, 0, 0, 0,           // dstX, dstY, srcX, srcY
      $this->max_xm, $this->max_ym,       // dstW, dstH
      $orig_x, $orig_y);    // srcW, srcH
	...
	function custom_tool_last_modified()
	{
	$houradd=date('H')+1;
